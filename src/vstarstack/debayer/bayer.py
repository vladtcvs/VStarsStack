...
def process(argv):
	if len(argv) > 0:
		...
		process_path([vstarstack.cfg.config["paths"]["npy-orig"], vstarstack.cfg.config["paths"]["npy-fixed"]])
...
def run(argv):
	vstarstack.usage.run(argv, "debayer bayer", commands, autohelp=False)
