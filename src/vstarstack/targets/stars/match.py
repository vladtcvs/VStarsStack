...
def best_ditem_match(ditem, descriptor, used, max_angle_diff, max_dangle_diff, max_size_diff):
    ...
    for i in range(len(descriptor)):
        if i in used:
            continue
        ditem2 = descriptor[i]
        matched, d = items_compare(
            ditem, ditem2, max_angle_diff, max_dangle_diff, max_size_diff)
        if not matched:
            continue
...
def descriptor_match(desc1,
    ...
    used = []
    for ditem in desc1:
        matched_i, _ = best_ditem_match(
            ditem, desc2, used, max_angle_diff, max_dangle_diff, max_size_diff)
...
def find_star_match(star,
    ...
    for i in range(len(stars)):
        ...
        matched = descriptor_match(
            desc1, desc2, max_angle_diff, max_dangle_diff, max_size_diff, min_matched_ditems)
        if matched:
            return i
    return None
...
def build_match(image1,
    ...
    for i in range(len(main1)):
        ...
        if "matches" not in star:
            star["matches"] = {}
        matched = find_star_match(
            star, main2, used, max_angle_diff, max_dangle_diff, max_size_diff, min_matched_ditems)
...
def matchStars(project: vstarstack.cfg.Project, description, descriptions):
    name = description[0]
    ...
    max_angle_diff = project.stars["match"]["max_angle_diff"] * fov
    ...
    for name_i, desc_i in descriptions:
        if debug:
            ...
            print("\n")
        desc = build_match(desc, desc_i, name_i, max_angle_diff,
                           max_dangle_diff, max_size_diff, min_matched_ditems)
    return name, desc
