...
def process(argv):
	if len(argv) >= 3:
		...
		flat_file = vstarstack.cfg.config["calibration"]["dark"]["path"]
...
def prepare_darks(argv):
	if len(argv) >= 2:
		...
		npys = vstarstack.cfg.config["calibration"]["dark"]["npy"]
		result = vstarstack.cfg.config["calibration"]["dark"]["path"]
...
def run(argv):
	vstarstack.usage.run(argv, "image-fix dark", commands, autohelp=False)
