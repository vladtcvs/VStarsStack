...
def fix(image, image_weight_layer, proj, image_weight=1):
	...
	if vstarstack.cfg.distorsion is None:
	...
	a = vstarstack.cfg.distorsion["a"]
	b = vstarstack.cfg.distorsion["b"]
	c = vstarstack.cfg.distorsion["c"]
	...
	for y in range(h):
		for x in range(w):
			...
			_, pixel_weight = vstarstack.common.getpixel(image_weight_layer, fy, fx, False)
...
def run(argv):
	vstarstack.usage.run(argv, "image-fix distorsion", commands)
