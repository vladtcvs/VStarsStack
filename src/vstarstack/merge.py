...
def simple_add(project: vstarstack.cfg.Project, argv: list):
    if len(argv) > 0:
        path_images = argv[0]
        out = argv[1]
    else:
...
def sigma_clip(project: vstarstack.cfg.Project, argv: list):
    if len(argv) > 0:
        path_images = argv[0]
        out = argv[1]
...
def run(project: vstarstack.cfg.Project, argv: list):
    vstarstack.usage.run(project, argv, "merge", commands, autohelp=True)
