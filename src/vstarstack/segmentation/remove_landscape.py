...
ROOT_PATH = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), "landscape_model")

sourcepath = sys.argv[1]
resultpath = sys.argv[2]

files = vstarstack.common.listfiles(sourcepath, ".npy")
...
for name, filename in files:
    ...
    for y in range(hc):
        ...
        for x in range(wc):
            posx = x * size
            lt = vstarstack.common.prepare_image_for_model(
                image[posy:posy+size, posx:posx+size])
            rt = vstarstack.common.prepare_image_for_model(
                image[posy:posy+size, w-posx-size:w-posx])
            lb = vstarstack.common.prepare_image_for_model(
                image[h-posy-size:h-posy, posx:posx+size])
            rb = vstarstack.common.prepare_image_for_model(
                image[h-posy-size:h-posy, w-posx-size:w-posx])
    ...
    np.save(os.path.join(resultpath, name + ".npy"), cimage)
