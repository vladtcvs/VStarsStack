...
for name, filename in files:
	...
	for y in range(hc):
		...
		for x in range(wc):
			posx = x * size
			lt = vstarstack.common.prepare_image_for_model(image[posy:posy+size, posx:posx+size])
			rt = vstarstack.common.prepare_image_for_model(image[posy:posy+size, w-posx-size:w-posx])
			lb = vstarstack.common.prepare_image_for_model(image[h-posy-size:h-posy, posx:posx+size])
			rb = vstarstack.common.prepare_image_for_model(image[h-posy-size:h-posy, w-posx-size:w-posx])
