...
We can assume that $p(\left\{ F_i \right\}) = \prod_i p(F_i)$ because frames are independent.
...
\begin{array}{r@{}l}
    ...
    p(f | \left\{ F_i \right\}) = \frac{\prod_i p(F_i | f) \cdot p(f)}{\prod_i \int p(F_i | f') p(f') df'} \\[9pt]
...
\begin{eqnarray}
    ...
    p(f_{signal} | \left\{ F_i \right\}) = \frac{\prod_i p(F_i | f_{signal}) \cdot p(f_{signal})}{\prod_i \int p(F_i | f_{signal}') p(f_{signal}') df_{signal}'}
\end{eqnarray}
...
So we can write:
\begin{eqnarray}
    E\left[ f_n \right] = \int df_n f_n \cdot p(f_n | \left\{ F_{i,1}, F_{j,2} \right\})
\end{eqnarray}

But the next step make a lot of effort, if we try to write
\begin{eqnarray}
    p(f_n | \left\{ F_{i,1}, F_{j,2} \right\}) = \frac{p(\left\{ F_{i,1}, F_{j,2} \right\} | f_n) \cdot p(f_n)}{p(\left\{ F_{i,1}, F_{j,2} \right\})}
\end{eqnarray}

we get value $p(\left\{ F_{i,1}, F_{j,2} \right\} | f_n)$ which require integration over $f_c$:
\begin{equation}
    p(\left\{F_{i,1}, F_{j,2} \right\} | f_n)= \int p(\left\{F_{i,1}, F_{j,2}\right\} | f_n, f_c) \cdot p(f_c) df_c
\end{equation}
...
\begin{eqnarray}
    ...
    p\left(\left\{F_{i,1}, F_{j,2} \right\} \right) = \prod_i p\left(F_{i,1} \right) \cdot \prod_j p\left(F_{j,2}\right)
\end{eqnarray}
