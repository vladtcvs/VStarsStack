...
\begin{array}{r@{}l}
    ...
    e^{-\sum_i \left(\lambda_i(f')-\lambda_i(f)\right)} \cdot \left( \frac{p(f')}{p(f)} \right) df'\right)^{-1} = \\[9pt]
    = \left( \int e^{\sum_i \left( F_i ln \lambda_i(f') - \lambda_i(f')\right) - \left( F_i ln \lambda_i(f) - \lambda_i(f)\right)} \cdot \left( \frac{p(f')}{p(f)} \right) df' \right)^{-1}
\end{array}
\end{equation}
...
\begin{eqnarray}
    F_{i,1} = Poisson(\lambda_{1,i}({f_n \atop f_c})) = Poisson\left( f_{dark, 1, i} + \nu_{1,i} \cdot \left( f_{sky, 1, i} + f_c + f_n \right) \right) \\
    F_{i,2} = Poisson(\lambda_{2,i}({f_n \atop f_c})) = Poisson\left( f_{dark, 2, i} + \nu_{2,i} \cdot \left( f_{sky, 2, i} + k \cdot f_c + f_n \right) \right)
\end{eqnarray}
...
\begin{eqnarray}
    ...
    = \frac{\prod_i \lambda_{i,1}({f_n \atop f_c})^{F_{i,1}} e^{-\lambda_{i,1}({f_n \atop f_c})} }{\prod_i F_{i,1}!} \cdot \frac{\prod_j \lambda_{j,2}({f_n \atop f_c})^{F_{j,2}} e^{-\lambda_{j,2}({f_n \atop f_c})}}{\prod_j F_{j,2}!} \\
    p\left(\left\{F_{i,1}, F_{j,2} \right\}\right) = \int p\left(\left\{F_{i,1}, F_{j,2} \right\} | {f'_n \atop f'_c}\right) \cdot p\left( f'_n \atop f'_c \right) df'_n df'_c = \nonumber \\
    = \int \frac{\prod_i \lambda_{i,1}({f'_n \atop f'_c})^{F_{i,1}} e^{-\lambda_{i,1}({f'_n \atop f'_c})} }{\prod_i F_{i,1}!} \cdot \frac{\prod_j \lambda_{j,2}({f'_n \atop f'_c})^{F_{j,2}} e^{-\lambda_{j,2}({f'_n \atop f'_c})}}{\prod_j F_{j,2}!} \cdot p\left({f'_n \atop f'_c}\right) df'_n df'_c
\end{eqnarray}
...
\begin{eqnarray}
    p\left({f_n \atop f_c} | \left\{ F_{i,1}, F_{j,2} \right\}\right) = \\
    \mathclap{= \left( \int 
    \prod_i \left( \frac{\lambda_{i,1}({f'_n \atop f'_c})}{\lambda_{i,1}({f_n \atop f_c})} \right)^{F_{i,1}} \cdot 
    \prod_j \left( \frac{\lambda_{j,2}({f'_n \atop f'_c})}{\lambda_{j,2}({f_n \atop f_c})} \right)^{F_{j,2}} \cdot 
    e^{-\sum_i \left(\lambda_{i,1}({f'_n \atop f'_c}) - \lambda_{i,1}({f_n \atop f_c})\right)
       -\sum_j \left(\lambda_{j,2}({f'_n \atop f'_c}) - \lambda_{j,2}({f_n \atop f_c})\right)} df'_n df'_c
    \right)^{-1}} \nonumber
\end{eqnarray}
